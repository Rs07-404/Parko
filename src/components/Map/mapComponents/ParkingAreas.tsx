...
const ParkingAreas = () => {
  ...
  const ParkingAreaPopUp: React.FC<{
    ...
    return (
      <Popup className="custom-popup" closeButton={false}>
        <Card className="text-sm w-full">
          <CardHeader className="p-0 pb-0 w-full flex justify-center">
            {parkingArea.name}
          </CardHeader>
          ...
          <CardContent className="flex flex-col p-0 pt-0 items-center justify-center gap-2">
            <div className="flex w-full items-center justify-center gap-1">
              Spots Available:{" "}
              <span className="text-green-500">{availableSpots}</span>
            </div>
            <div className="flex w-full items-center justify-center">
              <Button
                type="button"
                onClick={() =>
                  authUser?.currentReservation
                    ? toast.error("You already have an ongoing reservation")
                    : dispatch(selectParkingArea(parkingArea))
                }
                variant={"secondary"}
              >
                View Spots
  ...
  return (
    <>
      {(allParkingAreas?.length ?? 0) > 0 &&
        allParkingAreas.map((area) => {
          ...
          const availableSpots =
            area.parkingSpots?.filter((spot) => spot.status === "available") ??
            [];

          const Icon = getMarkerIcon(
            availableSpots.length > 0 ? "available" : "occupied"
          );

          // Defensive check for boundary coordinates
          const areaBounds = Array.isArray(area?.boundary?.coordinates?.[0])
            ? area.boundary.coordinates[0].map(
                ([lng, lat]: number[]) => [lat, lng] as [number, number]
              )
            : [];

          return (
            <Marker key={area._id} position={[lat, lng]} icon={Icon}>
              <ParkingAreaPopUp
                parkingArea={area}
      ...
      >
        <RefreshCcw
          className={`${
            AllAreaLoading && "text-primary transition-all animate-pulse"
          }`}
        />
      </Button>
    </>
